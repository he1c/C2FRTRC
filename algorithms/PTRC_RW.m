function Xt=PTRC_RW(M,Mask,I,option)
...
for k=1:1:N
    ...
    Mask_temp=permute(Mask==1,order);
    Mask_temp=reshape(Mask_temp,prod(J(order(1:d))),[]);
    B{k}=zeros(size(Mask_temp,1));
    sum_Mask_temp=sum(Mask_temp(:));
    for i=1:1:size(Mask_temp,1)
        temp=Mask_temp(i,:);
        B{k}(i,i)=max(sum(temp)/sum_Mask_temp,1e-16);
    end
...
for kk=1:1:maxitr
    ...
    for k=1:N
       ...
       %H{k}=inv(W{k}'*W{k}+1e-10*eye(size(W{k},2)))*W{k}'*X{k};
       X{k}=W{k}*H{k};
    end
