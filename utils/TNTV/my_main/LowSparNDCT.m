function [L1 S1 k eta beta] = LowSparNDCT(Y,Omega,opts)
...
for k = 1:itemax
    ...
    %% update L^{k+1}, F^{k+1}
    L1 = prox_dcttnn((Obser + M0 - S0 - Z12 -(T10 + T30)/beta)/2,1/(2*beta));
    
    diff_M     = diff3(M0(:), sizeD); 
    F1 = prox_l1tntv(diff_M - T20(:)/beta,gamma/beta);
    
    %% update Z^{k+1}
    Z1 = (beta*(Obser - L1 -S0) - T10).*Omega/(beta + rho) + (-L1 - S0 - T10/beta).*BarOmega;
    
    %% update S^{k+1}, M^{k+1}
    S1 = prox_l1tntv(Obser - L1 - Z1 - T10/beta,lambda/beta);
   ...
   %% stop criterion
   if k >= 100
   etal = L1 - prox_dcttnn(L1 - T1 - T3,1);
   etal = norm(etal(:))/(1 + norm(L1(:)) + norm(T1(:)) + norm(T3(:)));
   
   etaf = F1 - prox_l1tntv(F1 - T2,gamma);
   etaf = norm(etaf(:))/(1+ norm(F1(:)) + norm(T2(:)));
   
   etas = S1 - prox_l1tntv(S1 - T1,lambda);
